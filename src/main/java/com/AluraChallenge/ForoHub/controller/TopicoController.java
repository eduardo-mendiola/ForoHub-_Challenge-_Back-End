...
import com.AluraChallenge.ForoHub.domain.topico.DatosCrearTopico;
import com.AluraChallenge.ForoHub.domain.topico.DatosTopico;
...
public class TopicoController {
    ...
    @GetMapping("/{id}")
    public ResponseEntity mostrarTopicoRespuestas(@PageableDefault(size = 5) Pageable paginacion,
    ...
    @GetMapping("/noResolutos")
    public ResponseEntity<Page<DatosTopico>> listarTopicosNoResolutos(@PageableDefault(size = 5) Pageable paginacion) {
        var response = service.listarTopicosNoResolutos(paginacion);
        return ResponseEntity.ok(response);
    }
